...
function updateResults() {
	...
	for(var i = 0; i < SamoanToEnglishWords.length; i++ ){
		...
		var dictOrig = SamoanToEnglishWords[i][0].replace(/ʻ|'|`/g,"").toLowerCase();
		...
		if (dict != -1) {
				...
				if(useFreeMatch){
					...
					} else // If not using macron then just a simple unconstrained match
					smatchList.push(i);	
				} else if(dict == 0) {
		...
		// The stuff below will eventially go away when we use a unified single dictionary
